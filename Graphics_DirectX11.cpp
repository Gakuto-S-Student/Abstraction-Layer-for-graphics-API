...
bool GraphicsDirectX11::CreateRenderTargetView()
{
	...
	// Create render target view
	ret = m_device->CreateRenderTargetView(renderTarget.Get(), nullptr, &m_renderTargetView);
	if (FAILED(ret))
		return false;
...
bool GraphicsDirectX11::CreateDepthStencilView(const int width, const int height)
{
	...
	ret = m_device->CreateDepthStencilView(depthStencil.Get(), &viewDesc, &m_depthStencilView);
	if (FAILED(ret))
		return false;
